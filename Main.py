...
dl = DEAPLearningSystem(func_list=func_set, ngens=15, algorithm="earlyswitcher")
import pandas as pd
weightlist = [0 for i in range(10)]
no_examples = 1
for i in range(no_examples):
    ...
    for weight in weightlist:
        ...
        df = trainer.predict_equations(dl, no_train_samples=10, eqs=["distance"], input_range=(0, 100), use_gens=True)
        temp = df.loc[0, :]
        temp["Weight"] = weight
    ...
    final_df.to_csv(f"DistanceSwitcher{i}.csv", index=False)
